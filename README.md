...
## Programming
On the programming side I diverged a bit from the SFX Sound Expander and rather followed the AdLib style: the board only uses two addresses for the audio part: $FDE4 and $FDE5. The former is for writing the number of the YM register to be modified while the latter is for the value. The former address can also be read and it will return the YM status register, which is only useful for the detection of the board or if you want to make use of the YM internal timers.

Please note that, due to chip limitations, **you must wait at least 3.3 microseconds after you wrote the address, before you write the data, and then at least 23 microseconds before the next write**. These are the same limitations of an AdLib sound card, and in fact any AdLib programming tutorials (like [this one](https://bochs.sourceforge.io/techspec/adlib_sb.txt)) should be 100% applicable to SoundX just as well, except for the different addresses of course. The chip detection from that document also works!
...
Addresses can still be changed if someone convinces me to happy :).
